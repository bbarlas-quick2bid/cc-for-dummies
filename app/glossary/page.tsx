...
const terms: { term: string; definition: string; section?: string }[] = [
  ...
  { term: "Token", definition: "The unit an LLM works in. Not quite a word, more like a word chunk. 'unbelievable' is one token; 'I am happy' is three. Models have a maximum number of tokens they can process at once (the context window), and they charge based on how many tokens you use.", section: "How AI Works" },
  ...
  { term: "Training / Fine-tuning", definition: "Training is how an LLM is built by feeding it enormous amounts of text so it learns patterns. Fine-tuning is additional training on a smaller, specific dataset to specialize the model for a particular task or style.", section: "How AI Works" },
  ...
  { term: "nvm (Node Version Manager)", definition: "A tool for installing and switching between different versions of Node.js. Strongly recommended over installing Node directly, as it puts everything in your home directory so you never need sudo.", section: "Setup" },
  ...
  { term: "React", definition: "A JavaScript library for building user interfaces with reusable components. Next.js is built on React, so every Next.js page is a React component.", section: "Hello World" },
  ...
  { term: "TypeScript", definition: "JavaScript with type annotations. Instead of just writing x = 5, you write x: number = 5. TypeScript catches type errors before your code runs. Claude Code writes valid TypeScript, so you don't need to master it to use this guide.", section: "Hello World" },
  ...
  { term: "CDN (Content Delivery Network)", definition: "A network of servers distributed around the world that serve your site's files from the location closest to each visitor. Vercel uses a CDN, which is why a site hosted in the US loads fast in Europe too.", section: "Hello World" },
